...
function obtenerProductosEnCarrito()
{
    ...
    $sentencia = $bd->prepare("SELECT productos.id, productos.raza, productos.descripcion, productos.precio
     FROM productos
...
{
    $bd = obtenerConexion();
    $sentencia = $bd->query("SELECT id, raza, descripcion, precio FROM productos");
    return $sentencia->fetchAll();
}
...
function guardarProducto($raza, $precio, $descripcion)
{
    $bd = obtenerConexion();
    $sentencia = $bd->prepare("INSERT INTO productos(raza, precio, descripcion) VALUES(?, ?, ?)");
    return $sentencia->execute([$raza, $precio, $descripcion]);
}
